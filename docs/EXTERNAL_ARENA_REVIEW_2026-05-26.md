...
This review cross-checks the POC against three external arena allocator discussions:

- Ginger Bill, "Memory Allocation Strategies - Part 2": linear arena basics, power-of-two alignment, `prev_offset` for resize, free-all/reset, and temporary savepoints.
- Ryan Fleury, "Untangling Lifetimes": arenas are primarily a lifetime-design tool; scratch arenas need conflict handling so temporary allocations are not accidentally returned as persistent data.
- Ng Song Guan, "High Performance Memory Management: Arena Allocators": fixed-capacity arenas should either fail, grow by chained blocks, or fall back to `malloc`, but growing/fallback behavior adds management complexity.
